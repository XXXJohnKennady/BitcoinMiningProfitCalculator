...
wantsList.addEventListener('click', async (e) => {
  ...
  // Initialize Firebase and other required imports...

// DOM Elements
const experienceFill = document.getElementById('experience-fill');
const experienceLevelText = document.createElement('span'); // Create level display
experienceLevelText.id = 'experience-level';
experienceFill.parentElement.appendChild(experienceLevelText); // Append next to bar
...
function updateExperienceBar(exp) {
    ...
    const level = Math.floor(exp / experiencePerLevel);
    const progress = (exp % experiencePerLevel) / experiencePerLevel * 100; // Percentage of the bar filled
    
    experienceFill.style.width = `${progress}%`; // Set the width of the fill
    experienceLevelText.textContent = `Level ${level}`; // Display current level
}
...
// Call updateExperienceBar whenever experience is modified
async function updateExperience(newExp) {
    const user = auth.currentUser;
    if (user) {
        try {
            ...
            updateExperienceBar(newExp); // Update bar immediately
        } catch (error) {
            console.error("Error updating experience:", error);
        }
    }
